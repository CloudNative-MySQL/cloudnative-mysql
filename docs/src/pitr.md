...
description: "Architecture, component responsibilities, recovery flow, RPO/RTO model, and risks for CNMySQL PITR."
sidebar_position: 10
---

# Point-In-Time Recovery architecture

This document explains how CNMySQL implements point-in-time recovery (PITR) for
...
`xtrabackup_binlog_info` in the restored data directory. CNMySQL reads the GTID
...
from CNMySQL's perspective; the next archive pass retries it.
...
During restore, CNMySQL loads `_index.json` and plans replay from the base backup
...
sequenceDiagram
    ...
    Note over Init: Write .cnmysql-pitr-done sentinel
...
CNMySQL's PITR RPO is bounded by the archived GTID frontier, not by the base
backup time.
...
- Recovery replay is reentrant. After successful replay, CNMySQL writes
  `.cnmysql-pitr-done` in the data directory. If the init container retries, it
